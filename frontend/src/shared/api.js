...
// export const apis = {
//   apiDefault: apiModule("https://f1rstweb.shop/"),
//   apiSocial: apiModule("https://codingtestrg.shop/")
// }
...
export const apiSocial = axios.create({
  baseURL: "https://codingtestrg.shop/",
  timeout: 1000,
  headers: {
    "content-type": "application/json;charset=UTF-8",
...
apiSocial.interceptors.request.use(
  function (config) {
    const token = localStorage.getItem("token");
    const refreshToken = localStorage.getItem("refreshToken");
    try {
...
apiSocial.interceptors.response.use(
  function (response) {
    console.log("응답 성공! ", response);
